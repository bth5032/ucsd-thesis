...
\section{Background Estimation Methods} \label{sec:background_estimation_methods}
  ...
  \subsection{Data-Driven Predictions}
    Every background prediction we make is based on countless assumptions. Our assumptions come in two forms:
  ...
  \subsection{Z + Hadronic} \label{sec:z_+_hadronic}
    ...
    There is no ideal way to correct for these differences. When this method was incepted\cite{MET_templates}, the original author constructed fine-grained bins based on the number of jets within particular \pt windows. In our updated method, we found that simply correcting the \pt distribution of the $\gamma$+jets events gave a \MET profile with good enough agreement to the Z+jets sample. This is attained by deriving a weight for each photon event used in the prediction such that the \pt distribution of the $\gamma+$jets events is identical to the Z+jets data. The precise algorithm for this is described further below, but one other complication needs to be addressed before we can get there.
    
    \subsubsection{Contamination From Events With Real \MET (Electroweak Subtraction)} \label{sec:ewk_subtraction}
      This method is based on the idea that the \MET profile in $\gamma$+jets events will be identical to the \MET profile in Z+jets events due to an identical mechanism for generating artificial \MET in both populations. 

    \subsubsection{Algorithm For The Prediction}

      Now that we have addressed the fundamental premise of this method and all the complications 

    \begin{enumerate}
      \item We take a population of single photon + jets events which roughly pass the same criteria passed by the dilepton events, but with the requirement for two leptons replaces by the requirement for one photon.\footnote{precise definition in sec \ref{sec:met_templates_control_region}} 
      ...
      \item Create a 'clean' Z+jets sample by taking the dilepton data and subtracting the other background predictions from any distribution of interest (in this case \pt).
    ...
    The important takeaway is that corrections

    The leading confounding factors are the jet energies themselves as well as their orientation. For instance

    The ECAL provides a good measurement of photon energy, so given events with just  We correct for the number of jets in the event, up to a

    The jet mismeasurements are oriented along the axis of the jet for each 

    \begin{figure}[h!]
      \centering
      \includegraphics[width=\textwidth]{figures/MET_templates_idea.pdf}
      \caption{The central idea behind the Z+Hadronic background prediction. A well measured photon acts as a proxy for a well-measured Z boson (reconstructed by its leptonic decay products), then the \MET distribution for the events is a function of }
      \label{fig:met_templates_idea}
    \end{figure}


  \pt of the Z greater than 25 GeV for photon trigger.
  Generally, there is an expectation that the leptons, and therefore the reconstructed Z, will have much better energy resolution than the jets, and so the Therefore, we expect the \MET distribution for these types of events to be well modeled by the \MET distribution in events with a single photon

    \subsubsection{Electroweak Subtraction Closure Test} \ref{sec:ewk_subtraction_closure_test}
...
\section{Signal Interpretations} 
  ...
  \subsection{Statistical Treatment}
    ...
    \subsubsection{The Likelihood of an Observation}
      ...
      \begin{table}[!h]
        ...
        \end{center}
      \end{table}
    \subsubsection{Setting Exclusion Limits}
  ...
  \subsection{Systematics in Signal Yield} \label{sec:systematics_in_signal_yield}
    ...
    The following are the list of nuisance parameters for the expected yields of used when computing the likelihood of the data given the hypothesis that the signal exists. For background we could use these but instead just went with the closure test, also fast-sim shit doesn't exist for the other MC.
